...
int searchDirectoryForTiles(FileGrid* pfg, ChestGrid* pcg, const wchar_t* tilePath, size_t origTPLen, int verbose, int alternate, boolean topmost)
{
	...
	if (hFind == INVALID_HANDLE_VALUE)
	{
		...
		do {
			if (verbose) {
				wprintf(L"File %s in directory %s being examined", ffd.cFileName, tilePath);
			}
			// is it a directory?
		...
		if (found) {
			...
			if (pcg->cr[fullIndex].exists) {
				...
				if (wcscmp(origTileName, pcg->cr[fullIndex].fullFilename) == 0) {
					wprintf(L"WARNING: duplicate chest file ignored. File '%s' in directory '%s' is in a different location for the same texture '%s' in '%s'.\n", origTileName, tilePath, pcg->cr[fullIndex].fullFilename, pcg->cr[fullIndex].path);
				}
				else if (verbose) {
					wprintf(L"WARNING: duplicate chest file ignored. File '%s' in directory '%s' is a different name for the same texture '%s' in '%s'.\n", origTileName, tilePath, pcg->cr[fullIndex].fullFilename, pcg->cr[fullIndex].path);
				}
				else {
					wprintf(L"WARNING: duplicate chest file ignored. File '%s' is a different name for the same texture '%s'.\n", origTileName, pcg->cr[fullIndex].fullFilename);
				}
				return FILE_NOT_FOUND;
			}
			else {
